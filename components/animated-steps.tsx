...
const steps: Step[] = [
  {
    id: 1,
    title: "Visit the Event Aura Dashboard",
    isCompleted: true,
    content: {
      steps: [
        "Sign up for an account and log in to the Event Aura dashboard.",
  ...
  },
  {
    ...
    content: {
      steps: [
        "Fill in the required fields to register your event.",
        "Select the event type, date, and location.",
        "Provide a brief description of the event.",
        "Choose the number of attendees and ticket prices.",
...
const StepsOne = () => {
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-7">
        <div className="space-y-2">
          <CardTitle className="text-2xl font-bold">Steps For Hosting Your Event</CardTitle>
          <p className="text-muted-foreground">
            Complete the steps to host your event successfully
          </p>
        </div>
        <Switch
          checked={enabled}
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
          ...
          <div className="flex-1 h-1 bg-muted rounded-full">
            <div
              className="h-full bg-primary rounded-full transition-all duration-300"
              style={{
                width: `${(expandedStep ? expandedStep / totalSteps : 0) * 100}%`,
              }}
            />
          </div>
        </div>
